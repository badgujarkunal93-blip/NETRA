    ...
    >
      ...
      <div className="absolute top-3 left-3 right-3 z-[1000] flex items-center justify-between pointer-events-none gap-2">
        ...
        <div className="pointer-events-auto relative">
          <div className="flex items-center gap-2 bg-white border border-[#0B2341]/20 shadow-md px-3 py-1.5 rounded-lg text-xs">
            <FolderSearch className="w-4 h-4 text-[#F5B800]" />
            <input
              type="text"
              ...
              className="w-72 bg-transparent text-[#071A33] placeholder-[#071A33]/50 text-xs focus:outline-none font-sans font-bold"
            />
            {caseData && (
              <span className="text-[9.5px] font-mono px-1.5 py-0.5 rounded bg-[#FFFBEB] text-[#D97706] border border-[#F5B800]/50 uppercase font-bold">
                {caseData.status}
              </span>
            )}
            {selectedCaseId && (
              <button
                ...
                className="text-[#071A33]/60 hover:text-[#071A33] p-0.5"
          ...
          {isCaseSearchOpen && (
            <div className="absolute top-full left-0 w-96 mt-1.5 bg-white border border-[#0B2341]/20 rounded-md shadow-2xl py-1 z-50 max-h-72 overflow-y-auto divide-y divide-[#0B2341]/10">
              <div className="px-3 py-1.5 text-[9.5px] font-mono uppercase text-[#071A33]/70 border-b border-[#0B2341]/10 flex items-center justify-between font-bold bg-[#F4F7FB]">
                <span>Select Active Investigation FIR</span>
                <span className="text-[#D97706]">{filteredCases.length} Registered</span>
              </div>
              {filteredCases.map((c) => (
                <button
                  key={c.id}
                  onClick={() => handleSelectCase(c.id)}
                  className={`w-full text-left px-3 py-2 hover:bg-[#F4F7FB] flex items-center justify-between text-xs transition-colors ${
                    selectedCaseId === c.id ? 'bg-[#FFFBEB] border-l-4 border-[#F5B800]' : ''
                  }`}
                >
                  <div className="min-w-0 pr-2">
                    <div className="font-bold text-[#071A33] truncate flex items-center gap-1.5">
                      <span className="text-mono font-bold text-[#071A33]">{c.crime_no}</span>
                      <span className="text-[#071A33]/60 font-normal">({c.case_no})</span>
                    </div>
                    <div className="text-[10px] text-[#475569] font-medium truncate mt-0.5">
                      {c.police_station} • {c.crime_major_head}
                    </div>
                  </div>
                  <ChevronRight className="w-3.5 h-3.5 text-[#071A33]/50 flex-shrink-0" />
                </button>
              ))}
            </div>
          )}
      ...
      {isFilterPanelOpen && (
        <div className="absolute top-16 left-3 z-[1000] w-64 bg-white border border-[#0B2341]/20 rounded-lg p-3 text-xs space-y-3 text-[#071A33] shadow-2xl">
          <div className="flex items-center justify-between border-b border-[#0B2341]/10 pb-1.5">
            <span className="font-bold uppercase tracking-wider text-[10px] text-[#071A33]">
              Geospatial Filters
            </span>
            <button onClick={() => setIsFilterPanelOpen(false)} className="text-[#071A33]/60 hover:text-[#071A33]">
              <X className="w-3.5 h-3.5" />
            </button>
          </div>
          ...
          {/* Provenance Filter */}
          <div>
            <span className="text-[10px] font-mono uppercase text-[#475569] font-bold block mb-1.5" title="Filter between confirmed evidence and AI pattern clues">
            ...
            <div className="grid grid-cols-3 gap-1 text-[10px]">
              {[
                ...
                <button
                  ...
                  className={`py-1.5 px-1 rounded text-center font-mono font-bold transition-colors ${
                    provenanceFilter === prov.id
                      ? 'bg-[#071A33] text-white shadow-xs'
                      : 'bg-[#F4F7FB] text-[#071A33] hover:bg-[#E2E8F0] border border-[#0B2341]/10'
                  }`}
          ...
          <div className="pt-2 border-t border-[#0B2341]/10">
            <div className="flex items-center justify-between mb-1">
              <span className="text-[10px] font-mono uppercase text-[#475569] font-bold" title="Hide connections below this certainty threshold">
                Min Match Certainty
              </span>
              <span className="font-mono font-bold text-[#D97706] text-[11px]">{minConfidence}%</span>
            </div>
            <input
              type="range"
              min="0"
              ...
              className="w-full accent-[#F5B800] cursor-pointer bg-slate-200"
            />
          </div>
        </div>
      )}
      ...
      <div className="w-full h-full flex-1 relative">
        {selectedCaseId && caseData ? (
            ...
            {edges.map((edge) => {
              ...
              return (
                <Polyline
                  ...
                  pathOptions={{
                    color: isConnectedToSelected ? '#DC2626' : isInferred ? '#F5B800' : '#38BDF8',
                ...
                >
                  <Tooltip sticky direction="top" className="night-ops-tooltip">
                    <div className="font-mono text-[9.5px] uppercase tracking-wider">
                      <span className="text-[#F5B800] font-bold">{edge.verb || edge.label}</span>: {edge.detailLabel || edge.label} <span className="text-slate-300">({edge.confidence || 85}% Conf)</span>
                    </div>
                  </Tooltip>
                </Polyline>
              );
                ...
                >
                  <Tooltip direction="bottom" offset={[0, 12]} opacity={0.98} className="night-ops-tooltip">
                    <div className="font-mono text-[10px] uppercase tracking-wider">
                      <div className="font-bold text-[#F5B800] flex items-center gap-1">
                        <span>{node.label}</span>
                        {node.confidence && <span className="text-[8.5px] px-1 py-0.2 rounded bg-[#0E223D] text-slate-200">[{node.confidence}%]</span>}
                      </div>
                      <div className="text-slate-300 text-[9px] lowercase tracking-normal font-sans">{node.subtext}</div>
                    </div>
                  </Tooltip>
                </Marker>
              );
          ...
          <div className="flex flex-col items-center justify-center h-full text-center p-6 bg-[#061121] text-slate-300">
            <div className="w-14 h-14 rounded-full bg-[#0E223D] border border-[#1C3B64] flex items-center justify-center text-[#F5B800] mb-3 shadow-lg">
            ...
            <div className="flex items-center gap-2 mt-4">
              {casesList.slice(0, 3).map((c) => (
                <button
                  key={c.id}
                  onClick={() => handleSelectCase(c.id)}
                  className="px-3 py-1.5 bg-[#0E223D] hover:bg-[#132B4C] text-white text-xs font-mono font-bold rounded border border-[#1C3B64] transition-colors"
                >
                  {c.crime_no}
                </button>
              ))}
      ...
      <div className="absolute bottom-3 left-3 z-[1000] bg-[#071A33]/95 border border-[#0B2341] p-2.5 rounded shadow-2xl text-[10px] text-slate-200 backdrop-blur-md flex flex-col gap-1.5 pointer-events-auto">
        <div className="font-mono text-[9px] font-bold text-slate-300 uppercase tracking-wider flex items-center justify-between gap-3 border-b border-white/10 pb-1">
          <span>Night-Ops Telemetry</span>
          <span className="text-[#F5B800]">CIU-NET</span>
        </div>
        <div className="flex items-center gap-3 text-[9.5px] font-mono">
          <div className="flex items-center gap-1.5">
            <span className="w-2.5 h-2.5 rounded-full bg-[#DC2626] shadow-[0_0_8px_#DC2626] inline-block"></span>
            <span>Confirmed / Focal</span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="w-2.5 h-2.5 rounded-full bg-[#F5B800] shadow-[0_0_8px_#F5B800] inline-block"></span>
      ...
      {unplacedNodes.length > 0 && (
        <div className="absolute bottom-16 left-3 z-[1000] bg-white border border-[#0B2341]/20 px-3 py-1.5 rounded-lg text-[10px] text-[#071A33] flex items-center gap-2 shadow-lg">
          <AlertCircle className="w-3.5 h-3.5 text-[#D97706]" />
          <span className="font-mono font-bold text-[#071A33]">Unplaced Entities:</span>
          <div className="flex items-center gap-1">
            {unplacedNodes.map((u) => (
              <button
                key={u.id}
                onClick={() => setSelectedNode(u)}
                className="px-2 py-0.5 bg-[#071A33] hover:bg-[#0B2341] text-white rounded font-mono text-[9px] border border-[#071A33] font-bold"
              >
                {u.label}
              </button>
            ))}
      ...
      {/* 6. RIGHT SIDE DETAIL CARD (High Contrast, Crisp Dark Navy & Charcoal Typography) */}
      {selectedNode && (
        <aside className="absolute top-16 right-3 bottom-3 z-[1000] w-88 bg-white border border-[#0B2341]/20 rounded-lg overflow-hidden flex flex-col text-xs text-[#071A33] shadow-2xl">
          {/* Header */}
          <div className="p-3.5 bg-[#071A33] text-white border-b border-[#0B2341] flex items-start justify-between flex-shrink-0">
            <div className="min-w-0 pr-2">
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <span className="text-[9.5px] font-mono px-1.5 py-0.5 rounded bg-amber-50 text-[#D97706] border border-amber-200 font-bold">
              ...
              <div className="text-[10.5px] text-slate-200 truncate mt-0.5 font-medium">
          ...
          <div className="flex-1 overflow-y-auto p-3.5 space-y-3.5 divide-y divide-[#0B2341]/10">
            {/* SECTION 1: PROMINENT CONNECTIONS LIST */}
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <h3 className="text-[11px] font-bold uppercase tracking-wider text-[#071A33] flex items-center gap-1.5">
                  <Share2 className="w-3.5 h-3.5 text-[#F5B800]" />
                  Direct Connections ({detailedConnectionsList.length})
                </h3>
                <span className="text-[10px] text-[#475569] font-mono font-semibold">Click to fly to pin</span>
              </div>

              <div className="space-y-1.5 max-h-56 overflow-y-auto pr-1">
                {detailedConnectionsList.length === 0 ? (
                  <div className="p-3 text-center text-xs text-[#475569] italic bg-[#F4F7FB] rounded border border-[#0B2341]/10">
                  ...
                  detailedConnectionsList.map((conn) => {
                    ...
                    return (
                      <div
                        key={conn.edgeId}
                        className="rounded border border-[#0B2341]/15 hover:border-[#0B2341]/30 bg-[#F4F7FB] transition-all overflow-hidden shadow-2xs"
                      >
                        {/* Connection Header Row */}
                        <div
                          onClick={() => handleWalkToNode(conn.targetNode)}
                          className="p-2 flex items-center justify-between gap-2 cursor-pointer hover:bg-[#EAEFF8] transition-colors"
                        >
                          <div className="flex items-center gap-2 min-w-0">
                            ...
                            <div className="min-w-0">
                              ...
                              <div className="flex items-center gap-1.5 mt-0.5">
                                <span className="px-1 py-0.5 rounded bg-white font-mono font-bold text-[8.5px] text-[#071A33] uppercase border border-[#0B2341]/15 shadow-2xs">
                                  {conn.verb}
                                </span>
                                <span className={`text-[8.5px] font-mono font-bold uppercase ${
                                  conn.status === 'inferred' ? 'text-[#D97706]' : 'text-[#475569]'
                                }`}>
                          ...
                          <div className="flex items-center gap-1 flex-shrink-0">
                            ...
                            <button
                              ...
                              className="p-1 text-[#475569] hover:text-[#071A33] rounded"
                        ...
                        {isExpanded && (
                          <div className="p-2 bg-white border-t border-[#0B2341]/10 text-[10.5px] space-y-1 text-[#071A33]">
                            <div>
                              <strong className="text-[#071A33] font-mono uppercase text-[9.5px]">Description:</strong> {conn.detailLabel}
                            </div>
                            <div>
                              <strong className="text-[#071A33] font-mono uppercase text-[9.5px]">Evidence:</strong> {conn.evidence}
                            </div>
                            {conn.model_version && (
                              <div className="font-mono text-[#475569] text-[9.5px] font-medium">
            ...
            <div className="pt-3 space-y-1.5">
              <h3 className="text-[11px] font-bold uppercase tracking-wider text-[#071A33]">
                Identity & Key Attributes
              </h3>
              <div className="grid grid-cols-2 gap-2 text-[11px] bg-[#F4F7FB] p-2.5 rounded border border-[#0B2341]/12">
                <div>
                  <span className="text-[#475569] font-bold text-[10px] block">Entity ID:</span>
                  <span className="font-mono font-bold text-[#071A33]">{selectedNode.id}</span>
                </div>
                <div>
                  <span className="text-[#475569] font-bold text-[10px] block">Status:</span>
                  <span className="font-bold text-[#071A33]">{selectedNode.subtext || 'Active'}</span>
                </div>
                {selectedNode.lat && selectedNode.lng && (
                  <div className="col-span-2">
                    <span className="text-[#475569] font-bold text-[10px] block">Coordinates:</span>
                    <span className="font-mono font-bold text-[10.5px] text-[#D97706]">{selectedNode.lat.toFixed(4)}° N, {selectedNode.lng.toFixed(4)}° E</span>
                  </div>
                )}
                {selectedNode.dob && (
                  <div>
                    <span className="text-[#475569] font-bold text-[10px] block">DOB / Age:</span>
                    <span className="font-bold text-[#071A33]">{selectedNode.dob}</span>
                  </div>
                )}
                {selectedNode.owner && (
                  <div className="col-span-2">
                    <span className="text-[#475569] font-bold text-[10px] block">Beneficial Owner:</span>
                    <span className="font-bold text-[#071A33]">{selectedNode.owner}</span>
                  </div>
                )}
                {selectedNode.aliases && selectedNode.aliases.length > 0 && (
                  <div className="col-span-2">
                    <span className="text-[#475569] font-bold text-[10px] block">Known Aliases:</span>
                    <div className="flex flex-wrap gap-1 mt-0.5">
                      {selectedNode.aliases.map((al, idx) => (
                        <span key={idx} className="px-1.5 py-0.5 bg-white rounded border border-[#0B2341]/15 font-mono text-[9.5px] font-bold text-[#071A33] shadow-2xs">
                          "{al}"
                        </span>
                      ))}
                    </div>
            ...
            <div className="pt-3 space-y-1.5">
              <h3 className="text-[11px] font-bold uppercase tracking-wider text-[#071A33] flex items-center gap-1">
                <Sparkles className="w-3 h-3 text-[#F5B800]" />
                Case Intelligence
              </h3>
              <div className="p-2.5 bg-[#F4F7FB] border border-[#0B2341]/12 rounded text-[11px] text-[#071A33] space-y-1">
                <div>
                  <span className="font-bold text-[#D97706]">Case Correlation:</span>
                  <p className="mt-0.5 leading-snug font-medium text-[#071A33]">
            ...
            {selectedNode.events && selectedNode.events.length > 0 && (
              <div className="pt-3 space-y-1.5">
                ...
                <div className="space-y-2 relative pl-3.5 border-l-2 border-[#0B2341]/20 text-[10.5px]">
                  {selectedNode.events.map((evt, idx) => (
                    <div key={idx} className="relative">
                      <span className="absolute -left-[18px] top-1 w-2 h-2 rounded-full bg-[#F5B800] ring-2 ring-white"></span>
                      <div className="font-bold text-[#071A33]">{evt.event_type}</div>
                      <div className="text-[#475569] text-[9.5px] font-mono font-medium">
          ...
          {/* Card Footer Actions */}
          <div className="p-3 bg-[#F4F7FB] border-t border-[#0B2341]/12 flex flex-col gap-1.5 flex-shrink-0">
            <button
              onClick={() => {
              ...
              className="w-full py-2 bg-[#071A33] hover:bg-[#0B2341] text-white font-bold text-xs rounded transition-colors flex items-center justify-center gap-1.5 border border-[#071A33] shadow-xs cursor-pointer"
            >
              <span>Inspect Full Dossier</span>
              <ExternalLink className="w-3.5 h-3.5 text-[#F5B800]" />
            </button>
            {selectedNode.lat && selectedNode.lng && (
              <button
                onClick={() => setFlyToTarget({ lat: selectedNode.lat, lng: selectedNode.lng, t: Date.now() })}
                className="w-full py-1.5 bg-white hover:bg-[#EAEFF8] text-[#071A33] font-bold text-[11px] rounded border border-[#0B2341]/25 transition-colors cursor-pointer shadow-2xs"
              >
                Center Map on Pin
              </button>
            )}
      ...
      <div className="absolute bottom-3 left-3 z-[1000] bg-[#071A33]/95 border border-[#0B2341] px-3 py-1.5 rounded shadow-xl text-[10px] text-white flex items-center gap-4 backdrop-blur-md">
        <div className="flex items-center gap-1.5">
          <span className="w-3 h-0.5 bg-[#F5B800]"></span>
          <span className="font-semibold text-white">Observed Link</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="w-3 h-0.5 border-t border-dashed border-[#F5B800]"></span>
          <span className="font-semibold text-white">AI Inferred</span>
        </div>
        <div className="hidden sm:flex items-center gap-2.5 pl-3 border-l border-white/20">
          {Object.entries(ENTITY_CONFIG).map(([type, cfg]) => (
            <div key={type} className="flex items-center gap-1">
              ...
              <span className="text-[9px] font-mono text-white font-bold">{cfg.code}</span>
            </div>
          ))}
        </div>
      </div>
